...
const fallbackEvents: CMSItem[] = [
  { id: "fallback-asat", resource: "agenda", title: "Ujian ASAT Genap TA 2025/2026", description: "Ujian akhir semester berbasis CBT.", date: "2026-06-08", end_date: "2026-06-12", time: "08:00", image_url: null, link: "https://ujiango.smk-terataiputih2.sch.id/", is_published: true, code: null, badge: null, skills: [], careers: [], created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" },
  { id: "fallback-expo", resource: "agenda", title: "Pameran Karya Kreatif DKV & Expo RPL", description: "Pameran karya kreatif siswa.", date: "2026-06-20", end_date: null, time: "09:00", image_url: null, link: null, is_published: true, code: null, badge: null, skills: [], careers: [], created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" },
];
...
export function AcademicCalendar({ events, news }: AcademicCalendarProps) {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(displayEvents[0]?.id ?? null);
  const activeMonth = manualMonth ?? new Date(firstDate.getFullYear(), firstDate.getMonth(), 1);
  const selectedEvent = displayEvents.find((item) => item.id === selectedId) ?? displayEvents[0];
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.18fr_.82fr]" data-testid="academic-calendar">
      <div className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6" data-testid="calendar-month-card">
        ...
        <div className="mt-5 grid grid-cols-7 gap-1 text-center">{["Sen", "Sel", "Rab", "Kam", "Jum", "Sab", "Min"].map((day) => <span key={day} className="py-2 text-[10px] font-extrabold uppercase text-slate-400">{day}</span>)}{calendarDays.map((date, index) => {
