...
export default function Home() {
  ...
  useEffect(() => {
    const anyModalOpen = isPpdbOpen || isVideoOpen || Boolean(galleryVideo) || Boolean(facilityLightbox);
  ...
  }, [isPpdbOpen, isVideoOpen, galleryVideo, facilityLightbox]);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-16 text-slate-900 sm:pb-0" data-testid="school-website-shell">
      ...
      <main>
        ...
        {activeBanner && <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28" data-testid="spmb-banner-section"><Reveal className="overflow-hidden rounded-[1.75rem] bg-[#092c4c] text-white shadow-2xl"><div className="grid lg:grid-cols-2"><div className="relative min-h-[220px]">{activeBanner.image_url ? <img src={optimizeImageUrl(activeBanner.image_url, 900) ?? activeBanner.image_url} alt={activeBanner.title} className="absolute inset-0 h-full w-full object-cover" data-testid="spmb-banner-image" /> : <div className="absolute inset-0 bg-gradient-to-br from-[#0f4c81] to-[#092c4c]" />}</div><div className="flex flex-col justify-center p-7 sm:p-10">{activeBanner.badge && <span className="inline-flex w-fit items-center gap-2 rounded-full bg-amber-400/15 px-3.5 py-2 text-xs font-bold uppercase tracking-[.18em] text-amber-300"><Sparkles size={14} /> {activeBanner.badge}</span>}<h2 className="mt-5 font-heading text-2xl font-extrabold leading-tight sm:text-3xl">{activeBanner.title}</h2><p className="mt-2 text-sm leading-relaxed text-slate-300">{activeBanner.description}</p><div className="mt-6 flex flex-wrap gap-3"><Button onClick={() => setIsPpdbOpen(true)} className="h-12 rounded-full bg-amber-500 px-6 text-sm font-extrabold text-[#092c4c] hover:bg-amber-400" data-testid="spmb-banner-cta-button">Daftar sekarang <ArrowUpRight size={16} /></Button></div></div></div></Reveal></section>}
