...
export default function Home() {
  ...
  const [galleryVideo, setGalleryVideo] = useState<{ type: "youtube" | "file"; src: string; title: string } | null>(null);
  ...
  const displayGallery = rawGallery.map((item) => {
    ...
    const isFileVideo = !videoId && Boolean(item.image_url) && /\.(mp4|webm|ogg)(\?.*)?$/i.test(item.image_url ?? "");
    const image = videoId ? `https://img.youtube.com/vi/${videoId}/hqdefault.jpg` : item.image_url ?? imageUrls.hero;
    return { ...item, image, videoId, isFileVideo };
  });
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#f8fafc] pb-16 text-slate-900 sm:pb-0" data-testid="school-website-shell">
      ...
      <main>
        ...
        <section id="galeri" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28" data-testid="gallery-section"><div className="flex items-end justify-between"><div><p className="section-kicker">06 · Galeri aktivitas</p><h2 className="section-heading mt-3">Momen yang <span className="text-emerald-600">bermakna.</span></h2></div><a href="https://www.instagram.com/smkterput2.bekasi/" target="_blank" rel="noreferrer" className="hidden items-center gap-2 text-xs font-extrabold text-[#0f4c81] hover:text-emerald-600 sm:flex" data-testid="instagram-gallery-link"><Camera size={16} /> @smkterput2.bekasi <ArrowUpRight size={14} /></a></div><div className="mt-10 grid auto-rows-[145px] grid-cols-2 gap-3 sm:auto-rows-[170px] sm:grid-cols-4">{displayGallery.map((item, index) => {
              ...
              return isVideo
                ? <motion.button type="button" whileHover={{ scale: 1.015 }} onClick={() => setGalleryVideo({ type: item.videoId ? "youtube" : "file", src: item.videoId ?? (item.image_url ?? ""), title: item.title })} key={`${item.title}-${index}`} className={`group relative overflow-hidden rounded-2xl text-left ${sizeClass}`} data-testid={`gallery-item-${index + 1}`}>{inner}</motion.button>
                : <motion.a whileHover={{ scale: 1.015 }} href={item.link} target="_blank" rel="noreferrer" key={`${item.title}-${index}`} className={`group relative overflow-hidden rounded-2xl ${sizeClass}`} data-testid={`gallery-item-${index + 1}`}>{inner}</motion.a>;
            })}</div></section>
      ...
      <AnimatePresence>
        {showSpmbBanner && activeBanner && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-[#061e34]/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Pengumuman SPMB" data-testid="spmb-banner-modal"><motion.div initial={{ y: 20, scale: .97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: .97 }} className="relative w-full max-w-md overflow-hidden rounded-[1.75rem] bg-[#092c4c] text-white shadow-2xl"><button type="button" onClick={dismissSpmbBanner} className="absolute right-4 top-4 rounded-lg p-2 text-white/70 hover:bg-white/10 hover:text-white" aria-label="Tutup pengumuman" data-testid="spmb-banner-close-button"><X size={18} /></button>{activeBanner.image_url && <img src={activeBanner.image_url} alt={activeBanner.title} className="max-h-64 w-full object-contain bg-black/20" data-testid="spmb-banner-image" />}<div className="p-7 sm:p-8">{activeBanner.badge && <span className="inline-flex items-center gap-2 rounded-full bg-amber-400/15 px-3.5 py-2 text-xs font-bold uppercase tracking-[.18em] text-amber-300"><Sparkles size={14} /> {activeBanner.badge}</span>}<h2 className="mt-5 font-heading text-2xl font-extrabold leading-tight">{activeBanner.title}</h2><p className="mt-2 text-sm leading-relaxed text-slate-300">{activeBanner.description}</p><div className="mt-6 flex flex-wrap gap-3"><Button onClick={() => { setIsPpdbOpen(true); dismissSpmbBanner(); }} className="h-12 rounded-full bg-amber-500 px-6 text-sm font-extrabold text-[#092c4c] hover:bg-amber-400" data-testid="spmb-banner-cta-button">Daftar sekarang <ArrowUpRight size={16} /></Button><button type="button" onClick={dismissSpmbBanner} className="h-12 rounded-full border border-white/20 px-6 text-sm font-bold text-white hover:bg-white/10" data-testid="spmb-banner-dismiss-button">Nanti saja</button></div></div></motion.div></motion.div>}
