...
export default function Home() {
  ...
  const [showSpmbBanner, setShowSpmbBanner] = useState(() => { try { return sessionStorage.getItem("spmb-banner-dismissed") !== "1"; } catch { return true; } });
  const dismissSpmbBanner = () => { setShowSpmbBanner(false); try { sessionStorage.setItem("spmb-banner-dismissed", "1"); } catch { /* private browsing: ignore */ } };
  useEffect(() => {
  ...
  const displayGallery = galleryContent.data?.length ? galleryContent.data.map((item, index) => ({ title: item.title, category: item.description || "Kegiatan Sekolah", image: item.image_url ?? imageUrls.hero, link: item.link || "https://www.instagram.com/smkterput2.bekasi/", size: index === 0 ? "large" : index === 1 ? "tall" : index === 4 ? "wide" : "small" })) : fallbackGalleryItems.map((item) => ({ ...item, link: "https://www.instagram.com/smkterput2.bekasi/" }));
  const displayNews = [...(newsContent.data ?? [])].sort((a, b) => (b.date ?? "").localeCompare(a.date ?? ""));
  const heroImage = heroContent.data?.[0]?.image_url ?? null;
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f8fafc] pb-16 text-slate-900 sm:pb-0" data-testid="school-website-shell">
      ...
      <main>
          ...
          <div className="mx-auto grid min-h-[690px] max-w-7xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:py-20">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }} className="relative z-10" data-testid="hero-copy">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button onClick={() => setIsPpdbOpen(true)} className="h-12 rounded-full bg-amber-500 px-6 text-sm font-extrabold text-[#092c4c] shadow-xl shadow-amber-500/20 transition-all hover:-translate-y-1 hover:bg-amber-400" data-testid="hero-ppdb-button">Jelajahi SPMB 2027 <ArrowUpRight size={17} /></Button>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .7, delay: .15 }} className="relative" data-testid="hero-image-block">
              ...
              <div className="relative h-[420px] overflow-hidden rounded-[2rem] border border-white/20 bg-slate-800 shadow-2xl lg:h-[500px]">{heroImage && <img src={heroImage} alt="Siswa SMK belajar bersama" className="h-full w-full object-cover" />}<div className="absolute inset-0 bg-gradient-to-t from-[#092c4c]/70 via-transparent to-transparent" /><div className="absolute bottom-6 left-6 right-6 flex items-end justify-between"><span className="max-w-[220px] font-heading text-lg font-bold leading-tight text-white">Masa depan cerah,<br /><span className="text-amber-300">hidup pun indah.</span></span><span className="rounded-xl bg-white/15 px-3 py-2 text-[10px] font-bold text-white backdrop-blur-md">BEKASI · JAWA BARAT</span></div></div>
        ...
        <section id="berita-terkini" className="bg-slate-50 px-5 py-20 lg:px-8 lg:py-28" data-testid="news-section"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="section-kicker">05 · Berita sekolah</p><h2 className="section-heading mt-3">Kabar terbaru dari <span className="text-emerald-600">kami.</span></h2></div><p className="max-w-sm text-sm leading-relaxed text-slate-500">Ikuti pengumuman, prestasi, dan kegiatan terkini sekolah.</p></div><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3" data-testid="news-grid">{displayNews.map((item, index) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-testid={`news-item-${index + 1}`}><div className="flex items-center gap-2 text-[#0f4c81]"><Newspaper size={16} /><span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">{item.date ? new Date(item.date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) : "Berita"}</span></div><h3 className="mt-3 font-heading text-base font-extrabold text-[#0a3358]">{item.title}</h3><p className="mt-2 line-clamp-3 text-xs leading-relaxed text-slate-500">{item.description}</p>{item.link && <a href={item.link} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1 text-xs font-extrabold text-[#0f4c81] hover:text-emerald-600" data-testid={`news-item-link-${index + 1}`}>Baca selengkapnya <ArrowUpRight size={13} /></a>}</article>)}{displayNews.length === 0 && <div className="col-span-full rounded-2xl border border-dashed border-slate-300 p-10 text-center text-sm text-slate-400" data-testid="news-empty">Belum ada berita.</div>}</div></div></section>

        <section id="galeri" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28" data-testid="gallery-section"><div className="flex items-end justify-between"><div><p className="section-kicker">06 · Galeri aktivitas</p><h2 className="section-heading mt-3">Momen yang <span className="text-emerald-600">bermakna.</span></h2></div><a href="https://www.instagram.com/smkterput2.bekasi/" target="_blank" rel="noreferrer" className="hidden items-center gap-2 text-xs font-extrabold text-[#0f4c81] hover:text-emerald-600 sm:flex" data-testid="instagram-gallery-link"><Camera size={16} /> @smkterput2.bekasi <ArrowUpRight size={14} /></a></div><div className="mt-10 grid auto-rows-[145px] grid-cols-2 gap-3 sm:auto-rows-[170px] sm:grid-cols-4">{displayGallery.map((item, index) => <motion.a whileHover={{ scale: 1.015 }} href={item.link} target="_blank" rel="noreferrer" key={`${item.title}-${index}`} className={`group relative overflow-hidden rounded-2xl ${item.size === "large" ? "col-span-2 row-span-2" : item.size === "tall" ? "row-span-2" : item.size === "wide" ? "col-span-2" : ""}`} data-testid={`gallery-item-${index + 1}`}><img src={item.image} alt={item.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-[#092c4c]/80 via-transparent to-transparent opacity-80" /><div className="absolute bottom-3 left-3"><span className="text-[9px] font-bold uppercase tracking-widest text-amber-300">{item.category}</span><h3 className="mt-1 text-sm font-bold text-white">{item.title}</h3></div></motion.a>)}</div></section>

        <section id="kontak" className="bg-[#092c4c] px-5 py-20 text-white lg:py-28" data-testid="contact-section"><div className="mx-auto max-w-7xl lg:px-3"><div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]"><div><p className="section-kicker text-emerald-300">07 · Hubungi kami</p><h2 className="mt-3 max-w-lg font-heading text-3xl font-extrabold leading-tight sm:text-4xl">Langkah pertama menuju masa depan dimulai <span className="text-amber-300">di sini.</span></h2><p className="mt-5 max-w-md text-sm leading-relaxed text-slate-300">Tim kami siap membantu calon siswa dan orang tua mendapatkan informasi terbaik tentang program, fasilitas, dan SPMB.</p><div className="mt-8 space-y-4"><a href="https://wa.me/628211058321" target="_blank" rel="noreferrer" className="contact-line" data-testid="contact-whatsapp-primary"><span className="contact-icon bg-emerald-500/15 text-emerald-300"><MessageCircle size={18} /></span><span><small>WhatsApp SPMB</small><strong>+62 821-1058-321</strong></span><ArrowUpRight size={17} className="ml-auto" /></a><a href="mailto:reportterput2@gmail.com" className="contact-line" data-testid="contact-email-link"><span className="contact-icon bg-amber-500/15 text-amber-300"><Mail size={18} /></span><span><small>Email resmi</small><strong>reportterput2@gmail.com</strong></span><ArrowUpRight size={17} className="ml-auto" /></a><div className="contact-line" data-testid="contact-address-info"><span className="contact-icon bg-sky-500/15 text-sky-300"><MapPin size={18} /></span><span><small>Lokasi sekolah</small><strong>Jl. Rajawali V Perumnas 1, Bekasi Selatan</strong></span></div></div><div className="mt-6 overflow-hidden rounded-2xl border border-white/10" data-testid="contact-map"><iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3966.219862270376!2d106.97722267472493!3d-6.236668893760604!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2e698d21f23894cd%3A0x54ea838beac73d1d!2sSMK%20Teratai%20Putih%20Global%202%20Bekasi!5e0!3m2!1sid!2sid!4v1715569844943!5m2!1sid!2sid" width="100%" height="220" style={{ border: 0 }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Lokasi SMK Teratai Putih Global 2 Bekasi" data-testid="contact-map-iframe" /></div></div><div className="rounded-[1.75rem] bg-white p-6 text-slate-900 shadow-2xl sm:p-8" data-testid="contact-inquiry-card"><div className="flex items-start justify-between gap-4"><div><h3 className="font-heading text-xl font-extrabold text-[#0a3358]">Butuh informasi lebih lanjut?</h3><p className="mt-1 text-sm text-slate-500">Kirim pertanyaan, kami bantu jawab.</p></div><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-amber-600"><Send size={18} /></span></div><form onSubmit={submitContact} className="mt-7 grid gap-4 sm:grid-cols-2"><label className="sm:col-span-1"><span className="form-label">Nama lengkap</span><input required name="name" placeholder="Nama Anda" className="form-input" data-testid="contact-name-input" /></label><label className="sm:col-span-1"><span className="form-label">Nomor WhatsApp</span><input required name="phone" placeholder="08xx-xxxx-xxxx" className="form-input" data-testid="contact-phone-input" /></label><label className="sm:col-span-2"><span className="form-label">Pertanyaan</span><textarea required name="question" rows={3} placeholder="Apa yang ingin Anda ketahui?" className="form-input resize-none" data-testid="contact-question-input" /></label><button type="submit" disabled={createLead.isPending} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#0f4c81] px-5 text-sm font-extrabold text-white transition-all hover:-translate-y-0.5 hover:bg-[#0a3358] disabled:opacity-60 sm:col-span-2" data-testid="contact-submit-button">{createLead.isPending ? "Menyimpan…" : <>Kirim pertanyaan <ArrowRight size={16} /></>}</button></form></div></div></div></section>
      </main>

      <footer className="bg-[#061e34] px-5 py-8 text-slate-400" data-testid="site-footer"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-5 sm:flex-row sm:items-center lg:px-3"><div className="flex items-center gap-3"><img src="/logo-tp2.png" alt="Logo SMK Teratai Putih Global 2" className="h-9 w-9 rounded-xl object-contain" /><span><strong className="block text-sm text-white">SMK Teratai Putih Global 2</strong><small className="text-[10px]">Unggul · Terpercaya · Profesional</small></span></div><div className="flex items-center gap-4"><a href="https://www.instagram.com/smkterput2.bekasi/" target="_blank" rel="noreferrer" className="transition-colors hover:text-white" aria-label="Instagram" data-testid="footer-instagram-link"><Camera size={17} /></a><a href="https://www.youtube.com/watch?v=DtyvJvqRdpY" target="_blank" rel="noreferrer" className="transition-colors hover:text-white" aria-label="Youtube" data-testid="footer-youtube-link"><Video size={18} /></a><Link to="/admin" className="ml-1 text-[10px] font-bold hover:text-white" data-testid="footer-admin-link">Admin</Link><span className="border-l border-white/10 pl-4 text-[11px]">© 2026 etosdev.com</span></div></div></footer>
      ...
      <AnimatePresence>
        {showSpmbBanner && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-[#061e34]/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Pengumuman SPMB" data-testid="spmb-banner-modal"><motion.div initial={{ y: 20, scale: .97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: .97 }} className="relative w-full max-w-md overflow-hidden rounded-[1.75rem] bg-[#092c4c] p-7 text-white shadow-2xl sm:p-8"><button type="button" onClick={dismissSpmbBanner} className="absolute right-4 top-4 rounded-lg p-2 text-white/70 hover:bg-white/10 hover:text-white" aria-label="Tutup pengumuman" data-testid="spmb-banner-close-button"><X size={18} /></button><span className="inline-flex items-center gap-2 rounded-full bg-amber-400/15 px-3.5 py-2 text-xs font-bold uppercase tracking-[.18em] text-amber-300"><Sparkles size={14} /> SPMB 2027 / 2028</span><h2 className="mt-5 font-heading text-2xl font-extrabold leading-tight">Pendaftaran murid baru resmi dibuka.</h2><p className="mt-2 text-sm leading-relaxed text-slate-300">Amankan kursimu di 5 program keahlian unggulan kami, lengkap dengan kesempatan beasiswa prestasi.</p><div className="mt-6 flex flex-wrap gap-3"><Button onClick={() => { setIsPpdbOpen(true); dismissSpmbBanner(); }} className="h-12 rounded-full bg-amber-500 px-6 text-sm font-extrabold text-[#092c4c] hover:bg-amber-400" data-testid="spmb-banner-cta-button">Daftar sekarang <ArrowUpRight size={16} /></Button><button type="button" onClick={dismissSpmbBanner} className="h-12 rounded-full border border-white/20 px-6 text-sm font-bold text-white hover:bg-white/10" data-testid="spmb-banner-dismiss-button">Nanti saja</button></div></motion.div></motion.div>}
        {isPpdbOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-[#061e34]/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Formulir SPMB" data-testid="ppdb-modal"><motion.div initial={{ y: 20, scale: .97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: .97 }} className="relative w-full max-w-lg rounded-[1.75rem] bg-white p-6 shadow-2xl sm:p-8"><button type="button" onClick={() => setIsPpdbOpen(false)} className="absolute right-4 top-4 rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Tutup formulir SPMB" data-testid="ppdb-modal-close-button"><X size={18} /></button><p className="section-kicker">SPMB 2027 / 2028</p><h2 className="mt-2 font-heading text-2xl font-extrabold text-[#0a3358]">Mulai perjalananmu.</h2><p className="mt-2 text-sm leading-relaxed text-slate-500">Isi data singkat berikut. Tim SPMB kami akan menghubungi Anda melalui WhatsApp.</p><form onSubmit={submitPpdb} className="mt-6 space-y-4"><label className="block"><span className="form-label">Nama calon siswa</span><input required name="student" className="form-input" placeholder="Tulis nama lengkap" data-testid="ppdb-student-name-input" /></label><label className="block"><span className="form-label">Pilihan program keahlian</span><select name="major" className="form-input" defaultValue="RPL" data-testid="ppdb-major-select"><option value="RPL">RPL — Rekayasa Perangkat Lunak</option><option value="DKV">DKV — Desain Komunikasi Visual</option><option value="BD">BD — Bisnis Digital &amp; Pemasaran</option><option value="MP">MP — Manajemen Perkantoran</option><option value="AKL">AKL — Akuntansi Keuangan Lembaga</option></select></label><label className="block"><span className="form-label">Nomor WhatsApp orang tua / siswa</span><input required name="whatsapp" className="form-input" placeholder="08xx-xxxx-xxxx" data-testid="ppdb-whatsapp-input" /></label><button type="submit" className="mt-2 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-amber-500 text-sm font-extrabold text-[#092c4c] transition-all hover:bg-amber-400" data-testid="ppdb-submit-button">Kirim minat pendaftaran <ArrowRight size={17} /></button></form></motion.div></motion.div>}
