...
export default function Home() {
  ...
  const displayGallery = rawGallery.map((item) => {
    ...
    const image = videoId ? `https://img.youtube.com/vi/${videoId}/hqdefault.jpg` : driveId ? `https://lh3.googleusercontent.com/d/${driveId}` : item.image_url ?? imageUrls.hero;
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f8fafc] pb-16 text-slate-900 sm:pb-0" data-testid="school-website-shell">
      ...
      <footer className="bg-[#061e34] px-5 pb-8 pt-12 text-slate-400" data-testid="site-footer"><div className="mx-auto max-w-7xl lg:px-3"><div className="grid gap-8 sm:grid-cols-[1.2fr_1fr]"><div className="flex items-center gap-3"><img src="/logo-tp2.png" alt="Logo SMK Teratai Putih Global 2" className="h-9 w-9 rounded-xl object-contain" /><span><strong className="block text-sm text-white">SMK Teratai Putih Global 2</strong><small className="text-[10px]">Unggul · Terpercaya · Profesional</small></span><span className="ml-2 h-9 w-px bg-white/10" /><img src="/logo-yayasan.png" alt="Logo Yayasan Teratai Putih Global" className="h-14 w-14 object-contain" data-testid="footer-foundation-logo" /></div><div data-testid="footer-shortcuts"><p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Pintasan</p><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3">{footerShortcuts.map(([label, href]) => <a key={label} href={href} target="_blank" rel="noreferrer" className="text-xs font-bold text-slate-300 transition-colors hover:text-white" data-testid={`footer-shortcut-${label.toLowerCase().replaceAll(" ", "-")}`}>{label}</a>)}</div></div></div><div className="mt-8 flex flex-col justify-between gap-5 border-t border-white/10 pt-6 sm:flex-row sm:items-center"><div className="flex items-center gap-4"><a href="https://www.instagram.com/smkterput2.bekasi/" target="_blank" rel="noreferrer" className="transition-colors hover:text-white" aria-label="Instagram" data-testid="footer-instagram-link"><Camera size={17} /></a><a href="https://www.youtube.com/watch?v=DtyvJvqRdpY" target="_blank" rel="noreferrer" className="transition-colors hover:text-white" aria-label="Youtube" data-testid="footer-youtube-link"><Video size={18} /></a><Link to="/admin" className="ml-1 text-[10px] font-bold hover:text-white" data-testid="footer-admin-link">Admin</Link></div><span className="text-[11px]">© 2026 etosdev.com</span></div></div></footer>
