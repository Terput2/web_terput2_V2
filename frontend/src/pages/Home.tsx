...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f8fafc] pb-16 text-slate-900 sm:pb-0" data-testid="school-website-shell">
      ...
      <AnimatePresence>
        {showSpmbBanner && activeBanner && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-[#061e34]/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Pengumuman SPMB" data-testid="spmb-banner-modal"><motion.div initial={{ y: 20, scale: .97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: .97 }} className="relative w-full max-w-md overflow-hidden rounded-[1.75rem] bg-[#092c4c] p-7 text-white shadow-2xl sm:p-8"><button type="button" onClick={dismissSpmbBanner} className="absolute right-4 top-4 rounded-lg p-2 text-white/70 hover:bg-white/10 hover:text-white" aria-label="Tutup pengumuman" data-testid="spmb-banner-close-button"><X size={18} /></button>{activeBanner.badge && <span className="inline-flex items-center gap-2 rounded-full bg-amber-400/15 px-3.5 py-2 text-xs font-bold uppercase tracking-[.18em] text-amber-300"><Sparkles size={14} /> {activeBanner.badge}</span>}<h2 className="mt-5 font-heading text-2xl font-extrabold leading-tight">{activeBanner.title}</h2><p className="mt-2 text-sm leading-relaxed text-slate-300">{activeBanner.description}</p><div className="mt-6 flex flex-wrap gap-3"><Button onClick={() => { setIsPpdbOpen(true); dismissSpmbBanner(); }} className="h-12 rounded-full bg-amber-500 px-6 text-sm font-extrabold text-[#092c4c] hover:bg-amber-400" data-testid="spmb-banner-cta-button">Daftar sekarang <ArrowUpRight size={16} /></Button><button type="button" onClick={dismissSpmbBanner} className="h-12 rounded-full border border-white/20 px-6 text-sm font-bold text-white hover:bg-white/10" data-testid="spmb-banner-dismiss-button">Nanti saja</button></div></motion.div></motion.div>}
